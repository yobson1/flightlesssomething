...
function getLineChartOptions(title, description, unit, maxY = null) {
    return {
        ...
        tooltip: {
            ...
            pointFormat: `<span style="color:{series.color}">{series.name}</span>: <b>{point.y:.2f} ${unit}</b><br/>`
        },
        plotOptions: {
            line: {
                marker: {
...
const fpsAverages = fpsDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
const frametimeAverages = frameTimeDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
const cpuLoadAverages = cpuLoadDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
const gpuLoadAverages = gpuLoadDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
const gpuCoreClockAverages = gpuCoreClockDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
const gpuMemClockAverages = gpuMemClockDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
...
function getBarChartOptions(title, unit, maxY = null) {
    return {
        ...
        yAxis: {
            ...
            min: 0,
            max: maxY,
            title: { text: unit, align: 'high', style: { color: '#FFFFFF' } },
            labels: {
                ...commonChartOptions.yAxis.labels,
                formatter: function () {
                    return this.value.toFixed(2) + ' ' + unit;
...
function createBarChart(
    ...
    options.series = [
        { name: title, data: data, colorByPoint: true, colors: colors }
    ];
    Highcharts.chart(chartId, options);
}
...
createBarChart(
    ...
    'Average FPS',
    'fps',
    fpsDataArrays.map((dataArray) => dataArray.label),
    fpsAverages,
    colors
);
createBarChart(
    'frametimeSummaryChart',
    'Average Frametime',
    'ms',
    frameTimeDataArrays.map((dataArray) => dataArray.label),
    frametimeAverages,
    colors
);
createBarChart(
    ...
    'Average CPU Load',
    '%',
    cpuLoadDataArrays.map((dataArray) => dataArray.label),
    cpuLoadAverages,
    colors,
    100
);
createBarChart(
    ...
    'Average GPU Load',
    '%',
    gpuLoadDataArrays.map((dataArray) => dataArray.label),
    gpuLoadAverages,
    colors,
    100
);
createBarChart(
    'gpuCoreClockSummaryChart',
    'Average GPU Core Clock',
    'Hz',
    gpuCoreClockDataArrays.map((dataArray) => dataArray.label),
    gpuCoreClockAverages,
    colors
);
createBarChart(
    'gpuMemClockSummaryChart',
    'Average GPU Memory Clock',
    'Hz',
    gpuMemClockDataArrays.map((dataArray) => dataArray.label),
    gpuMemClockAverages,
    colors
);
...
const avgFPSData2 = fpsDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
...
function filterOutliers(data) {
    data.sort((a, b) => a - b);
    return data.slice(
        Math.floor(data.length * 0.01),
        Math.ceil(data.length * 0.97)
    );
}
...
function countFPS(data) {
    ...
    let fpsArray = Object.keys(counts)
        .map((key) => [parseInt(key), counts[key]])
        .sort((a, b) => a[0] - b[0]);
    ...
    while (fpsArray.length > 100) {
        ...
        fpsArray[minIndex][0] =
            (fpsArray[minIndex][0] + fpsArray[minIndex + 1][0]) / 2;
...
const densityData = fpsDataArrays.map((dataArray) => ({
    name: dataArray.label,
    data: countFPS(filterOutliers(dataArray.data))
}));
...
Highcharts.chart('fpsDensityChart', {
    ...
    xAxis: {
        ...commonChartOptions.xAxis,
        title: { text: 'FPS', style: { color: '#FFFFFF' } },
        labels: { style: { color: '#FFFFFF' } }
    }, // Show X-axis labels in white
    tooltip: {
        ...commonChartOptions.tooltip,
        shared: true,
        formatter: function () {
            return `<b>${this.points[0].series.name}</b>: ${this.points[0].y} points at ~${Math.round(this.points[0].x)} FPS`;
        }
    },
    plotOptions: {
        areaspline: { fillOpacity: 0.5, marker: { enabled: false } }
    },
    legend: { ...commonChartOptions.legend, enabled: true },
    series: densityData
});
...
Highcharts.chart('frametimeMinMaxAvgChart', {
    ...
    tooltip: {
        ...
        valueSuffix: ' ms',
        formatter: function () {
            return `<b>${this.series.name}</b>: ${this.y.toFixed(2)} ms`;
        }
    },
    plotOptions: {
        bar: {
...
const avgFrametimeData2 = frameTimeDataArrays.map((dataArray) =>
    calculateAverage(dataArray.data)
);
...
const sortedPercentageFrametimeData = sortedFrametimeData.map(
...
Highcharts.chart('frametimeAvgChart', {
    ...
    title: {
        ...
        text: 'Avg Frametime comparison in %'
    },
    subtitle: { ...commonChartOptions.subtitle, text: 'Less is better' },
    xAxis: {
        ...commonChartOptions.xAxis,
    ...
    series: [
        {
            name: 'Frametime Percentage',
            data: sortedPercentageFrametimeData,
            colorByPoint: true,
            colors: colors
        }
    ]
...
function countFrametime(data) {
    ...
    let frametimeArray = Object.keys(counts)
        .map((key) => [parseInt(key), counts[key]])
        .sort((a, b) => a[0] - b[0]);
    ...
    while (frametimeArray.length > 100) {
        ...
        frametimeArray[minIndex][0] =
            (frametimeArray[minIndex][0] + frametimeArray[minIndex + 1][0]) / 2;
...
// Calculate counts for each dataset after filtering outliers
const frametimeDensityData = frameTimeDataArrays.map((dataArray) => ({
    name: dataArray.label,
    data: countFrametime(filterOutliers(dataArray.data))
}));
...
Highcharts.chart('frametimeDensityChart', {
    ...
    xAxis: {
        ...commonChartOptions.xAxis,
        title: { text: 'Frametime (ms)', style: { color: '#FFFFFF' } },
        labels: { style: { color: '#FFFFFF' } }
    }, // Show X-axis labels in white
    tooltip: {
        ...commonChartOptions.tooltip,
        shared: true,
        formatter: function () {
            return `<b>${this.points[0].series.name}</b>: ${this.points[0].y} points at ~${Math.round(this.points[0].x)} ms`;
        }
    },
    plotOptions: {
        areaspline: { fillOpacity: 0.5, marker: { enabled: false } }
...
const frametimeSdvCategories = frameTimeDataArrays.map(
    (dataArray) => dataArray.label
);
const frametimeStandardDeviations = frameTimeDataArrays.map((dataArray) =>
    calculateStandardDeviation(dataArray.data)
);
const frametimeVariances = frameTimeDataArrays.map((dataArray) =>
    calculateVariance(dataArray.data)
);
...
Highcharts.chart('frametimeStddevVarianceChart', {
    ...
    subtitle: {
        ...commonChartOptions.subtitle,
        text: 'Measures of Frametime consistency (std. dev.) and spread (variance). Less is better.'
    },
    ...
    series: [
        {
            name: 'Std. Dev.',
            data: frametimeStandardDeviations,
            color: '#FF5733'
        },
